...
cat data/train_{30m,60m,clean,all}/text | cut -f2 -d" " | tr ' ' '\n' | sed 's/^[ \t]*//;s/[ \t]*$//' | sed '/^$/d' | sort -u > "$dir/vocab"
...
#paste $dir/vocab <(python3 -c "[print(' '.join(line.strip())) for line in open('$dir/vocab', encoding='utf-8')]") >> "$dir/lexicon.txt"
